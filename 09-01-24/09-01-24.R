...
ggplot() +
geom_col(data = merged_df, aes(x = prop_total_births, y = reorder(month, desc(month)), fill = data_id, color = data_id), width = 0.5, position = "dodge") + scale_x_continuous(expand = expansion(mult = c(0.05, 0.3))) + 
...
theme_void() +
theme(
    axis.text.y = element_text(size = 10, hjust = 1, color = "#293276", family = "Helvetica"),
    axis.text.x =  element_text(size = 10, hjust = 1, color = "#293276", family = "Helvetica", ),
    plot.title = element_text(size = 11, vjust = 2, color = "#293276", family = "Helvetica", face = "bold"),
    plot.subtitle = element_text(size = 9, vjust = 2, color = "#293276", family = "Helvetica"),
    plot.caption = element_text(size = 8, vjust =  -5, color = "#293276", family = "Helvetica"),
    plot.margin = margin(rep(15, 4)),
    axis.title.x =  element_text(size = 10, color = "#293276",vjust = -3, hjust = 0.3, family = "Helvetica-Narrow"),
    axis.title.y =  element_text(size = 10, angle = 90, color = "#293276", vjust = 3, family = "Helvetica-Narrow"),
    legend.text = element_text(size = 8, family = "Helvetica", color = "#293276"),
    legend.title = element_blank()
  ) +  geom_vline(xintercept = 1/12, linetype = "twodash", color = "#293276",linewidth = 0.75) +
  geom_label(data = annotation, aes( x = x, y = y, label = label),                 , 
             colour = "#293276", fontface = "italic", size = 3, angle = 45)+ scale_fill_manual(values = c("#999999", "#bf3f3f")) + scale_color_manual(values = c("black", "#C02C38"))
